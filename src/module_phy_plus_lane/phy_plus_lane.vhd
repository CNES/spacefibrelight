...
entity phy_plus_lane is
   port(
      ...
      CLK                              : in  std_logic;                       --! main clock for spacefibre IP (used also as freerunning clock for HSSL)
      -- Reset_gen interface
      LANE_RESET_PPL_OUT               : out std_logic; -- FIXME this signal probably is not supposed to be used and should be removed soon after it is removed from NGultra finalisation
      CLK_TX_OUT                       : out std_logic;                       --! Clock generated by manufacturer IP

      -- CLK GTY signals
      CLK_GTY                           : in std_logic;                        --! Clock for the extended phy layer IP
      RST_GTY_DONE                      : out std_logic;                       --! Up when GTY reset is done
...
signal aligned_capability                       : std_logic_vector(07 downto 00);   --! int3 received capability aligned with lane_init_fsm active state signal
...
signal reset_gty_all_in                         : std_logic;                        --! GTY IP reset
...
signal autorised_rx_valid                       : std_logic                     ;   --! Data valid flag generated by GTY IP copied when RX is authorized by lane init statemachine
...
signal INTF0_TX0_ch_txelecidle                  : std_logic                     ;   --! create electrical idle state
signal INTF0_RX0_ch_rxpd                        : std_logic_vector(01 downto 00);   --! Command to disable the receiver part of GTY IP
signal INTF0_rst_tx_done_out_0                  : std_logic;                        --! Up when internal GTY tx reset done
signal INTF0_rst_rx_done_out_0                  : std_logic;                        --! Up when internal GTY Rx reset done
signal INTF0_RX0_ch_rxelecidle                  : STD_LOGIC_VECTOR(0 DOWNTO 0);     --! value 1 when GTY RX is Out of band (normally only for PCIe /sata)
signal no_signal                                : std_logic;                        --! value 1 when GTY is in high Z (electrical idle) and the init lane state machine requires use of this signal

-- Internal signals from BufG_GT_wrapper
signal clk_usr_gty                                   : std_logic;                   --! Clock generated by the BufG_GT, image of QUAD0_TX0_outclk generated by GTY IP. it is used for both RX and TX communication with HSSL
-- ctrl internal signals
signal ctrl_in_dl,ctrl_in_dl_r  : std_logic_vector(8 downto 0); -- data and reset command from link layer to be clock domain change
...
signal rst_gty_done_i                      :  std_logic;            --! Up when GTY reset is done
...
signal lane_reset_cmd : std_logic;     -- contatenation of all source of lane reset cmd  (from MIB and link layer) using main IP clock
signal lane_reset_cmd_gty : std_logic; --contatenation of all source of lane reset cmd  (from MIB and link layer) after clock domain change mechanism

signal rst_n_i_gty,rst_n_i_gty_r : std_logic; -- rst_n reset resync in clk_usr_gty domain
signal rst_usr_gty_n             : std_logic; -- reset that will be used for every module using clk_usr_gty
...
-- PHY + lane reset management
   ...
   begin
      if RST_N='0' then
         ...
         rst_n_i_gty<='1';
         rst_n_i_gty_r<=rst_n_i_gty;
      end if;
   end process;
   rst_usr_gty_n<=rst_n_i_gty_r;

------------------------------------------------------------------------------
-- Clock domain change between Data link layer to lane layer
-- data link layer run at spacefibre clk frequency (CLK)
-- data lane layer run at clk tx HSSL frequence (clk_usr_gty)
------------------------------------------------------------------------------

   --- DATA change domain
   -- we use reset in the wr_clk domain => CLK signal
   --- We do not want to loose data so we pass trough a fifo for clock domain change
   ...
   --- CONTROL change domain
   --- we are interested in signal value. So we use double clock as clock domain change strategy
   ctrl_in_dl      <= LANE_RESET_DL & CAPABILITY_TX;

   p_cdc_command_from_link: process(clk_usr_gty)
   begin
      if rising_edge(clk_usr_gty) then
         ctrl_in_dl_r<=ctrl_in_dl;
         ctrl_in_dl_sync<=ctrl_in_dl_r;
      end if;
   end process;
   capability_tx_i <= ctrl_in_dl_sync(7 downto 0) ;
   lane_reset_cmd_dl_i <= ctrl_in_dl_sync(8);  -- lane reset from datalink

   lane_reset_cmd_gty<=lane_reset_cmd_dl_i or mib_lane_reset_r2;--contatenation of both lane reset (from datalink and MIB) in clk_usr_gty domain


   fifo_in_ctrl_data_valid<='1';
   ...
   ------------------------------------------------------------------------------
   -- valid RX data only if RX transmitter is enabled
   -- the HSSL doesn't allow a disabled state for RX excepted by doing a reset which result in connection error afteward)
   autorised_rx_valid<= INTF0_RX0_ch_rxdatavalid(0) and not(receiver_dis_from_lif);
   ...
   port map(
      ...
      -- FROM MANUFACTURER IP
      DATA_RDY_FROM_IP                 => autorised_rx_valid,
...
-- reset is performed in WR_CLK domain => clk_usr_gty
-- we do not want to loose data so we use fifo
   ...
   INTF0_TX0_ch_txelecidle    <= transmitter_dis_from_lif; -- when high disable TXN and TXP output
...
--------------- beginning of conditionnal port map ------------
   ...
   ------------ End of conditionnal port map---------------
     ...
     INTF0_RX0_ch_rxpd(1 downto 0) => "00",
     ...
     INTF0_TX0_ch_txelecidle(0) => INTF0_TX0_ch_txelecidle,
     ...
     INTF0_TX0_ch_txpd(1 downto 0) => "00",
     ...
     INTF0_rst_all_in => reset_gty_all_in_valid_r,
     INTF0_rst_rx_datapath_in => '0',
     INTF0_rst_rx_done_out => INTF0_rst_rx_done_out_0,
   ...
   -- no signal comes from the out of band signal from GTY and is considered only if initstate machine requires it
   no_signal <= no_signal_detection_enabled_from_lif and INTF0_RX0_ch_rxelecidle(0);
   ------------------------------------------------------------------------------
   ...
   port map (
      ...
      O => clk_usr_gty,          -- user output clock 150MHz
...
CLK_TX_OUT                 <= clk_usr_gty; --HSSL TX user clock domain
...
RX_ERROR_CNT               <= rx_error_cnt_from_lif;
RX_ERROR_OVF               <= rx_error_ovf_from_lif;
LOSS_SIGNAL                <= no_signal;
RX_POLARITY                <= invert_rx_bits_from_lif;
FAR_END_CAPA               <= aligned_capability;
lane_active_dl_i           <= enable_transm_data_from_lif;

rst_gty_done_i             <= INTF0_rst_tx_done_out_0 and INTF0_rst_rx_done_out_0;
RST_GTY_DONE               <= rst_gty_done_i;
...
-- therefore we clock domain change them with double clock process
 p_cdc_MIB_ctrl: process(clk_usr_gty)
 begin
    if rising_edge(clk_usr_gty) then
